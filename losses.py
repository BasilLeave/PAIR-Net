...
class SemanticLoss(nn.Module):
    ...
    def forward(self, visual, text, labels):
        # calculate the similarity between visual and text
        ...
        logits_per_image = torch.bmm(visual.float(), text.transpose(1, 2).float())
        logits_per_text = torch.bmm(text.float(), visual.transpose(1, 2).float())
        # print('logits_per_image:', logits_per_image.shape)
        # print('logits_per_text:', logits_per_text.shape)

        num_classes = labels.shape[-1]
        logits_per_image_reshape = logits_per_image.view(-1, num_classes)
        logits_per_text_reshape = logits_per_text.view(-1, num_classes)
        labels = labels.reshape(-1, num_classes)
        image_loss = F.cross_entropy(logits_per_image_reshape, labels)
        text_loss = F.cross_entropy(logits_per_text_reshape, labels)
...
class SAT(nn.Module):
    def __init__(self, args):
        ...
        self.targets = torch.zeros((args.num_train, args.num_class))
        self.epoch = 1
        self.Es = args.Es
