...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed, mask=None):
        ...
        if self.num_encoder_layers > 0:
            memory, enc_attn_maps = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
        else:
            memory, enc_attn_maps = src, None

        tgt = torch.zeros_like(query_embed)
        hs, attn_maps = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed)

        return hs.transpose(0, 1), memory[:h * w].permute(1, 0, 2), attn_maps, enc_attn_maps
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            output, attn_map = layer(output, src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask, pos=pos)
            encoder_attn_weights.append(attn_map)
