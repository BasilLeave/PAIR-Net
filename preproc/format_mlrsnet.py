...
for label_file in label_files:
    ...
    for i in range(num_images):
        ...
        if i < int(num_images * 0.8):
            image_paths_train.append(os.path.join(root_path, 'Images', label_name, row[0]))
            print(os.path.join(root_path, 'Images', label_name, row[0]))
            labels_train.append(row[1:].astype(np.float32))
            num_train += 1
        else:
            image_paths_test.append(os.path.join(root_path, 'Images', label_name, row[0]))
            print(os.path.join(root_path, 'Images', label_name, row[0]))
